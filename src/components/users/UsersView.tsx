...
export const UsersView: React.FC = () => {
  ...
  const handleDisable2FA = async (u: User) => {
    const ok = await confirm({
      title: '关闭双因素身份验证 (2FA)',
      message: `确定要关闭用户 [${u.username} (${u.email})] 的 2FA 保护吗？关闭后登录将不再要求输入动态验证码。`,
      confirmText: '确认关闭',
      type: 'warning'
    });
    if (!ok) return;
    ...
    try {
      await api.disable2FA(u.id);
      toast.success(`已为 [${u.username}] 关闭 2FA 验证`);
      fetchUsers();
    } catch (err: any) {
      toast.error(`关闭 2FA 失败: ${err.message}`);
    }
  };
  ...
  return (
    <div className="flex-1 flex flex-col justify-between space-y-6">
      <div className="space-y-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">
              支持本地账号独立 TOTP 2FA，同时支持为 AuthMate SSO 用户叠加平台独立 2FA 双重防护
            </p>
          </div>
        ...
        {/* Dual-Track 2FA Architecture Banner */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div className="p-3.5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 flex items-start gap-3 shadow-sm">
            <div className="w-8 h-8 rounded-xl bg-emerald-50 dark:bg-emerald-950/80 text-emerald-600 dark:text-emerald-400 flex items-center justify-center shrink-0 mt-0.5">
              <Smartphone className="w-4 h-4" />
            </div>
            <div className="min-w-0 flex-1 text-xs">
              <div className="font-bold text-slate-900 dark:text-white flex items-center gap-1.5">
                <span>① 本平台独立 2FA (RFC 6238 TOTP)</span>
                <span className="text-[10px] px-1.5 py-0.2 rounded bg-emerald-100 dark:bg-emerald-950/80 text-emerald-700 dark:text-emerald-300 font-medium">
                  全账号通用
                </span>
              </div>
              <p className="text-slate-500 dark:text-slate-400 mt-1 leading-relaxed text-[11px]">
                无论是<strong>本地管理员</strong>还是 <strong>AuthMate SSO 授权用户</strong>，均可在本平台独立绑定 Google / 微软验证器，登录时二次校验 6 位动态码。
              </p>
            </div>
          </div>

          <div className="p-3.5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 flex items-start gap-3 shadow-sm">
            <div className="w-8 h-8 rounded-xl bg-blue-50 dark:bg-blue-950/80 text-blue-600 dark:text-blue-400 flex items-center justify-center shrink-0 mt-0.5">
              <Key className="w-4 h-4" />
            </div>
            <div className="min-w-0 flex-1 text-xs">
              <div className="font-bold text-slate-900 dark:text-white flex items-center gap-1.5">
                <span>② AuthMate SSO 统一 2FA (IdP 托管)</span>
                <span className="text-[10px] px-1.5 py-0.2 rounded bg-blue-100 dark:bg-blue-950/80 text-blue-700 dark:text-blue-300 font-medium">
                  企业级单点
                </span>
              </div>
              <p className="text-slate-500 dark:text-slate-400 mt-1 leading-relaxed text-[11px]">
                支持在企业上游 AuthMate 统一身份认证中心开启全员 MFA 策略，实现短信/邮箱/WebAuthn 与本平台独立 2FA 灵活双轨运行。
              </p>
            </div>
          </div>
        </div>
              ...
              <table className="w-full text-left text-xs text-slate-600 dark:text-slate-400">
                <thead className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800 text-slate-800 dark:text-slate-300 font-bold">
                  <tr>
                    ...
                    <th className="py-3 px-4">2FA 双因素安全保护</th>
                    <th className="py-3 px-4">账号状态</th>
                    <th className="py-3 px-4">上次登录时间</th>
                  ...
                  {paginatedUsers.map(u => {
                    ...
                    return (
                      <tr key={u.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                        ...
                        {/* Universal 2FA Status Column (Applicable to Both Local and SSO) */}
                        <td className="py-3 px-4">
                          {u.twoFactorEnabled ? (
                            <div className="flex items-center gap-2">
                              <span className="inline-flex items-center gap-1 text-[11px] font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/60 px-2.5 py-1 rounded-xl border border-emerald-200/80 dark:border-emerald-800/60 shadow-sm">
                                <ShieldCheck className="w-3.5 h-3.5" />
                                <span>已启用独立 2FA</span>
                              </span>
                              <button
                                onClick={() => handleDisable2FA(u)}
                                className="text-[10px] text-slate-400 hover:text-rose-500 underline decoration-dotted ml-1"
                                title="关闭此用户的 2FA 认证"
                              >
                                关闭
                              </button>
                            ...
                            <button
                              ...
                              className="inline-flex items-center gap-1.5 text-[11px] font-semibold text-slate-600 dark:text-slate-300 hover:text-emerald-600 dark:hover:text-emerald-400 bg-slate-100 dark:bg-slate-800 hover:bg-emerald-50 dark:hover:bg-emerald-950/40 px-2.5 py-1 rounded-xl border border-slate-200/60 dark:border-slate-700/60 transition-all active:scale-95"
                              title={isSSO ? '为该 SSO 账号叠加本平台独立 2FA 双因素安全保护' : '为该本地账号绑定 2FA 身份验证器'}
                            >
      ...
      {filteredUsers.length > 0 && (
        <div className="mt-auto pt-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-4 py-3 bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 rounded-2xl shadow-sm text-xs text-slate-500 dark:text-slate-400">
            <div>
              显示第 {filteredUsers.length === 0 ? 0 : `${startIndex + 1}-${Math.min(startIndex + pageSize, filteredUsers.length)}`} 项，共 {filteredUsers.length} 项
            </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl w-full max-w-md p-6 space-y-5 shadow-2xl animate-scaleUp">
            ...
            <form onSubmit={handleSave} className="space-y-4 text-xs">
              ...
              <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700">
                <div>
                  <span className="font-bold block text-slate-800 dark:text-slate-200">启用该账号</span>
                  <span className="text-[11px] text-slate-400">停用后该用户将无法登录系统</span>
                </div>
                <input
                  type="checkbox"
                  checked={isActive}
              ...
              <div className="flex gap-2 pt-2">
                <button
                  ...
                  className="w-1/2 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 font-semibold"
                >
                  取消
                </button>
                <button
                  type="submit"
                  disabled={submitting}
                  className="w-1/2 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold shadow-md transition-all active:scale-95 disabled:opacity-50"
                >
                  {submitting ? '保存中...' : '确认保存'}
                </button>
              </div>
      ...
      {/* 2FA TOTP Setup Modal (Available to ALL Users: Local & SSO) */}
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl w-full max-w-md p-6 space-y-5 shadow-2xl animate-scaleUp">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="font-bold text-slate-900 dark:text-white text-base">配置独立 2FA 双因素身份验证</h3>
            ...
            <div className="space-y-4 text-xs">
              ...
              <div className="space-y-2 p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/80 dark:border-slate-700/60">
                <div className="font-bold text-slate-800 dark:text-slate-200 flex items-center gap-1.5">
                  ...
                  <span>在手机身份验证器中录入此密钥 (Secret Key)</span>
                </div>
                <p className="text-[11px] text-slate-500 dark:text-slate-400">
                  支持 Google Authenticator、Microsoft Authenticator、1Password、Bitwarden 等所有 RFC 6238 标准验证器。
                </p>
